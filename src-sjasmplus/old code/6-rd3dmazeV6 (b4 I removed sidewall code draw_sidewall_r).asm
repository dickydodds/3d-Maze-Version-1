...
; draw right half of the display.
        ...
        ld b,3; was 16          ; count for 3 columns left or right of display file
...
do_draw_4r:
        ...
        ld b,4                 ; count for max of 3 blocks right of maze position
...
wall4_start:
        ld a,b
        sub 3                   ;3 = wall immediately to the right
        jp z,draw_9r_2

        ld a,b
        sub 4                   ;2 = 2nd wall to the right
        jp z,draw_9r

        ld a,b
        sub 1                   ;1 = 3rd wall immediately to the right
        jp z,draw_9r
...
;move our start point to the end point of our first wall3
       ; inc de
;        inc de
;        inc de
        ;inc de
;below can be changed to ind hl
...
;;        dec a
;;        jr z,exit_1_2             ;exit after 2 loops
;;        ld (var7),a             ;save it
...
;;
;;        ld a,3                  ;reduce black wall column height
;;        ld  (var1),a            ;save the count for the column hight minus the top corners
...
;        jp z,draw_10r          ;draw an end wall if blockid=16
        ...
        ld a,b
        sub 2
        jp z,draw_10r         		;1 is an end wall so draw it
...
;#############################################
; draw 2nd layer right of section 3
;#############################################
draw_2r:
