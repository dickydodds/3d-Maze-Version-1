...
do_no_wall:
        pop de
        ;push de
        ld   a,_bottomblack;_topwhitebottomchequer
        ld   (de),a                             ; insert the top of the wall face.
        ld   hl,$0021
        add  hl,de                              ; advance to the next row of the display file.
        ld   (hl),_topblack;_topchequerbottomwhite        ; insert the bottom of the wall face.
...
; we only need to draw up to 2 columns of black sides.
...
cont_3L:
        ld de,33                ;to jump to next line below
        ld a,$ca                ;top of the wall
        push hl                 ;save for drawing 2nd black wall        
        ld (hl),a               ;to cover up wall 5
        ...
        ld (hl),a        
        add hl,de
        ld a,$cf                ;draw bottom of the line
        ld (hl),a        

;now check if we need to cover wall 5 with a 2nd black line to the left
;hl points to top of the 4 blocks
        pop hl                  ;restore our pointer in char ram
        inc hl                  ;go left 1 place
        push hl
        add hl,de               ;now we are at the wall 5 top position
        ld a,(hl)
        cp _smallwall
        pop hl
        jr nz,exit3L
;draw the second wall over section 5 wall
        ld (hl),$c9             ;top of the wall       
        add hl,de
        ld (hl),_black
        add hl,de
        ld (hl),_black
        add hl,de
        ld (hl),$ce             ;bottom of the wall

exit3L:  pop de                  ;now exit
        pop bc
        jp go_back4
