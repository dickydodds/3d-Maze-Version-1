...
a4_1:   cp 3
        ...
        ld b,a                  ;make the bc count 1 so we only draw 1 wall
        add a,128               ;make sure we set a wall!
        jp draw_9r_2L           ;draw the 2nd block to the left
...
a4_2:   cp 2
        ...
        ld b,a                  ;make the bc count 1 so we only draw 1 wall
        add a,128               ;make sure we set a wall!
        jp draw_9r_3L           ;draw the 3rd block to the left
...
a4_3:   
        ...
        ld b,a                  ;make the bc count 1 so we only draw 1 wall
        add a,128               ;make sure we set a wall!
        jp draw_9r_4L           ;draw the 4th block to the left
...
draw_9r_3L:  
        ...
        ld b,4                  ;block is 4 blocks high
        ld hl,$0021             ;load with 32 to jump to line below us
xx        ld de,$C162-24          ;start of wall drawing
...
;loop for 5 black wall columns
        ...
        ld b,6                  ;loop 5 times for 3 columns
        ld (var5),hl            ;address of first face block after corner chamfer
        ld a,$cd ;91
        ld (var2),a             ;ld var2 with top right sloping corner black
...
draw_9r_4L:  
        ...
        ld b,4                  ;block is 4 blocks high
        ld hl,$C18C-66              ;start of drawing
...
;need 2 black walls now!
        ld a,_black
        ld de,32                ;1 line width

        CALL black_wall         ;draw 2 black blocks        
...
;1 = 3rd left wall      
        ...
        ld a,b                  ;our current wall counter
        sub 3
        jp nz,a3_1                 ;its not wall 2
        ld a,1
        ld b,a
        add a,128               ;make sure we set a wall!
        jp draw_10             ;jump straight to making an end wall

a3_1:   ld a,b                  ;our current wall counter
        sub 2
        jp nz,a3_2                 ;its not wall 2
        ld a,1
        ld b,a
        add a,128               ;make sure we set a wall!
        jp draw_10_2L	       	;draw the 2nd wall to the left
...
a3_2:   ld a,1
        ld b,a                  ;make the bc count 1 so we only draw 1 wall
        add a,128               ;make sure we set a wall!
        jp draw_10_3L	       	;draw the 2nd wall to the left
...
draw_10_3L: 
         ...
         push bc
         push de
         ld hl, $c000+300-3    ;$c000+300-3          ;start print position of wall

;check if a wall is already drawn. If so, dont draw a black wall.
        ld a,(hl)
        cp $b5
        jp nz,j4
        pop de
        pop bc
        jp z,go_back3              ; jump back to main loop if brick already drawn
...
;is 8 x 8
    ...
    ;we need 1 line of 6 vertical chars
j4:     ld de,$21       ;for 1 line jump
        ld a,$ca        ;top corner
        ld (hl),a        
        add hl,de
        ld a,_black        
