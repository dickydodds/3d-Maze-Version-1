...
;              call get_distance ; get distance we can see
              ...
              ei                    ;enable inturrupts in case we return to BASOC
...
;will need to change DF_CC position for sjasmplus
        ...
        ld bc,$0204      ;set row and column to 2,4
rep1:   push bc
        call locate
        pop bc           ;restore our counter
        push bc          ;resave it
        ld a,_mediumwall;'#'         ;print a # char
        call setbase     ;point char set to text or graphics
        ld de,$f         ;blue background, white writing
        ld (att),de      ;load it for print routine
        ...
        jp nz,rep1
        ld a,b
        cp 19            ;draw 18 lines (starting at line 2)
        ret z            ;exit after 18 lines
        inc b
