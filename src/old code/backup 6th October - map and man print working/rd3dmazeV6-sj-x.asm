...
;              call compass         ; draw compass
            ...
            ;  call get_distance    ; got to get how far we can see again
              ...
              ;check if we can see an end wall. If so, draw walls left
              ;and right to simulate the long outside edge wall

              call draw_side_walls

;################################################################################
;below for version 2 with upper levels

;maybe we can just run load_walk once instead of of LOAD_DE every time we
;draw a layer as we already ran LOAD_DE and setup (left) & (right) and DE
;will contain how much we walk bak in the display

           ;   call load_walk        ;de holds the jump distance to the next block
                                    ;in the maze for walking forward or backward

           ;   call draw_upper_1     ;draw uper 1 middle ceiling
...
;for SJASMPLUS
    ...
    INCLUDE "7-u1-middle.asm" ; draw upper layer 1 middle
